<?php /*a:1:{s:33:"../app/view/admin/guige_page.html";i:1658491331;}*/ ?>
<!DOCTYPE html>
<head>
...
<body style="padding:0px;">
    ...
    <table id="dg" title="产品规格" class="easyui-datagrid" style="width:100%;height:auto"
            url="guige_list"
            toolbar="#toolbar"
    ...
    <div id="win">
        ...
        名称<input  style="width:300px" id="win_p_names"><br />
    ...
    function cc_add(){//主瓦尺寸新增
        wins(function(r){
            if(r){
                $.post('zhuwa_guige_add',r,function(res){
                    if(res.code != 1){
    ...
    function cc_del(){//主瓦尺寸删除
        ...
        if (row){
            $.messager.confirm('提示','该操作会使得与该规格相关的产品定价失效，确认删除吗?',function(r){
                if (r){
                    $.post('zhuwa_guige_del',{id:row.id},function(res){
                        if(res.code != 1){
                            $.messager.show({	// show error message
                                title: 'Error',
                                msg: res.msg,
    ...
    function cc_update(){
        ...
        if(row){
            ...
            wins(function(r){
                if(r){
                    r.id = row.id;
                    $.post('zhuwa_guige_update',r,function(res){
                        if(res.code != 1){
                            $.messager.show({	// show error message
                                title: 'Error',
                                msg: res.msg,
